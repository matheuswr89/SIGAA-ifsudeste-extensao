ab = Math.floor(Math.random() * (3 - 1)) + 1
...
if (ab == 1) {
        ...
        if (TABELA_ATIVIDADES != null) {
            ...
            for (let i = 0; i < tabelaAtividades.children.length; i++) {
                ...
                if (atividades.children[0].querySelector("img") != null) {
                    if (atividades.children[0].querySelector("img").getAttribute("src").includes("prova_semana")) {
                        tipo = "vermelho";
                        nEnviados++;
                    } else if (atividades.children[0].querySelector("img").getAttribute("src").includes("prova_mes")) {
                        tipo = "";
                        proximas++;
                    } else {
                        tipo = "verde";
          ...
          `;
            if (antigos > 0) {
                ...
                <div class="task" onclick="">
                  <p class="datas-task">${arrayAtividades[i][1]} ${(arrayAtividades[i][1] != "" && arrayAtividades[i][2] == "") ||
            ...
            if (nEnviados > 0) {
                for (let i = 0; i < arrayAtividades.length; i++) {
                    if (
                        ...
                        conteudoAtividades += `
                <div class="task" onclick="${arrayAtividades[i][3].querySelector("a") != null
                                ? arrayAtividades[i][3].querySelector("a").getAttribute("onclick")
                                : null
                            }">
                  <p class="datas-task">${arrayAtividades[i][1]} - ${arrayAtividades[i][2]
                            }</p>
                  ${arrayAtividades[i][3].innerHTML}
                </div>
            `;
    ...
    $(".task, .task a").click(function (e) {
        setTimeout(incrementarTesteAB('a'), 2000);
        console.log("Enviou para o firebase A");
    })

} else {
    $("#avaliacao-portal a").click(function (e) {
        setTimeout(incrementarTesteAB('b'), 2000);
        console.log("Enviou para o firebase B");
    })
}
